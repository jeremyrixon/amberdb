...
public class ChangedWorksTest {
    ...
    public void testGetChangedWorks() throws Exception {
        ...
        Assert.assertEquals(changed.size(), 84);
        
        changed = sess.getModifiedWorkIds(time2);
        Assert.assertEquals(changed.size(), 63);

        changed = sess.getModifiedWorkIds(time3);
        Assert.assertEquals(changed.size(), 42);
        
        changed = sess.getModifiedWorkIds(time4);
        Assert.assertEquals(changed.size(), 21);
        ...
        Assert.assertEquals(changed.size(), 4); // 3 deleted pages + 1 modified book
        ...
        Assert.assertEquals(changed.size(), 21); // 3 deleted pages + 1 modified book
    }        
