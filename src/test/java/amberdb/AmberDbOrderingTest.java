...
public class AmberDbOrderingTest extends AbstractDatabaseIntegrationTest {
    ...
    public void testPersistReorderedWork() throws IOException {
        ...
        try (AmberSession db = amberDb.resume(sessId)) {
            ...
            int i = 0;
            for (Page page : pages) {
                Assert.assertTrue(page.getId() == order.get(i).longValue());
                i = i + 1;
            }
            db.close();
        }
        try (AmberSession db = amberDb.resume(sessId)) {
            ...
            int i = 0;
            for (Node page : pages) {
                Assert.assertFalse(page.getId() == order.get(i).longValue());
                i = i + 1;
        ...
        try (AmberSession db = amberDb.resume(sessId)) {
            ...
            int i = 0;
            for (Work page : pages) {
                Assert.assertTrue(page.getId() == order1.get(i).longValue());
                i = i + 1;
            }
            db.close();
        }
